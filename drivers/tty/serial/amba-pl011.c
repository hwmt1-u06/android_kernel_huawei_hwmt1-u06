...
#define RUNTIME_RESUME_FLAG_BY_AP	(1U<<(2))
#define RUNTIME_RESUME_FLAG_BY_CP	(1U<<(3))
...
void amba_pl011_port_runtime_disable_by_cp(void);
void amba_pl011_port_runtime_enable_by_cp(void);
#endif
...
#ifdef CONFIG_ENABLE_K3V2_UART_SLEEP_CONTROL_FOR_D2
	uap->suspend_flags &= ~RUNTIME_RESUME_FLAG_BY_AP;
...
static inline void amba_pl011_port_disable(struct uart_amba_port *uap)
{
	...
	if((uap->suspend_flags & RUNTIME_SUSPEND_FLAG_ENABLE ) \
		&& !(uap->suspend_flags & RUNTIME_RESUME_FLAG_BY_AP) \
		&& !(uap->suspend_flags & RUNTIME_RESUME_FLAG_BY_CP))
	{
...
void amba_pl011_port_runtime_disable_by_cp(void)
{
	if (amba_ports[uart_port1] != NULL) {
		amba_ports[uart_port1]->suspend_flags &= ~RUNTIME_RESUME_FLAG_BY_CP;
		amba_pl011_port_runtime_disable_oneshot(amba_ports[uart_port1]);
	}
}
EXPORT_SYMBOL(amba_pl011_port_runtime_disable_by_cp);
...
void amba_pl011_port_runtime_enable_by_cp(void)
{
	if (amba_ports[uart_port1] != NULL) {
		amba_ports[uart_port1]->suspend_flags |= RUNTIME_RESUME_FLAG_BY_CP;
		amba_pl011_port_runtime_enable_oneshot(amba_ports[uart_port1]);
	}
}
EXPORT_SYMBOL(amba_pl011_port_runtime_enable_by_cp);
