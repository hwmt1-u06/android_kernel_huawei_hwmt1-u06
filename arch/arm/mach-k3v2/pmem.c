...
void __init k3v2_allocate_memory_regions(void)
{
	...
	/* GPU memory */
//for d2, the pmem total(13M Camera) exceeds vmalloc, so change the gpu reserve address to 1GB(0x40000000)
//for other product just use address below 1GB
#ifdef CONFIG_LCD_1080P
	//size = hisi_media_mem.gpu_size;
	hisi_reserved_gpu_phymem = 0x40000000;
	printk("pmem allocating %lu bytes at (%lx physical) for gpu "
		"pmem area\n", hisi_media_mem.gpu_size, reserved_base);
